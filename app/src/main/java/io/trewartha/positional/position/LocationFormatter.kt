...
class LocationFormatter(private val context: Context) {
    ...
    fun getAccuracy(location: Location?, metric: Boolean): String {
        val accuracy = when {
            ...
            else -> UnitConverter.metersToFeet(location.accuracy).toInt()
        }
    ...
    fun getElevation(location: Location?, metric: Boolean): String {
        val elevation = when {
            ...
            else -> UnitConverter.metersToFeet(location.altitude.toFloat()).toInt()
        }
    ...
    fun getSpeed(location: Location?, metric: Boolean): String {
        val speed = when {
            location == null -> 0.0f
            metric -> UnitConverter.metersPerSecondToKilometersPerHour(location.speed)
            else -> UnitConverter.metersPerSecondToMilesPerHour(location.speed)
        }
