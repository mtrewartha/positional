...
- **Domain Type Based Names**: Name repository interfaces by prefixing the word "Repository" with
  the domain type they vend. For example, a repository that vends `Foo` instances would be named "
  FooRepository".
- **Repository Interfaces**: Always define repository interfaces separate from repository
  implementations.
- **Data Sources**: When a single repository abstracts away multiple data storage mechanisms,
  abstract away each storage mechanism behind a "data source" interface.
...
- **Getter Functions**: Follow these conventions when defining functions that look data up:
    - If returning a `Flow` so the caller can observe a single item, then:
        ...
        - When the returned `Flow` is collected, it should emit the current item at the start of
          collection (or `null`
          if not found), then emit the item or `null` every time it changes to something distinct.
    ...
    - If returning a `Flow` so the caller can observe a list of items, then:
        ...
        - Set the return type to `Flow<Collection<T>>` where `Collection` is an appropriate
          `Collection` type (e.g.
          `Set`, `List`) and `T` is the domain type name, e.g. `Flow<List<Foo>>`.
    ...
    - If returning a list of items, then:
        ...
        - Set the return type to `Collection<T>` where `Collection` is an appropriate `Collection`
          type (e.g. `Set`,
          `List`) and `T` is the domain type name, e.g. `List<Foo>`.
